...
export class OvsalesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.fetchCustomerOverallSales();
  }
  ...
  fetchCustomerOverallSales(): void {
    this.cuService.getCustomerOverallSales().subscribe({
  ...
  applyCurrencyFilter(): void {
    this.filteredData = this.selectedCurrency === 'All'
      ? this.rowData
      : this.rowData.filter(d => d.currency === this.selectedCurrency);
  ...
  currencyFormatter(params: any): string {
    ...
    const symbol = this.currencySymbols[currencyCode.toUpperCase()] || currencyCode;
